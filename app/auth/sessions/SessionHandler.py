...
class SessionHandler:
    ...
    async def get_user_by_session(self, session_token):
        ...
        if session and session.user:
            return {"user_matric": session.user.user_matric,
    ...
    async def get_user_session_by_matric(self, user_matric:str):
        existing_user_session = await self.sessionRepository.get_user_session_by_matric(user_matric)
    ...
    async def create_new_session(self, user_matric:str):
        ...
        if existing_user_session:
            raise HTTPException(status_code=400, detail=f"Already logged in. Sign out of other devices before logging in again")
        
        token = await self.sessionRepository.create_new_session(session_token=session_token, EXPIRES_AT=EXPIRES_AT, user_matric=user_matric)
        return token
    
    async def deactivate_session(self, session_token):
        session_state = await self.sessionRepository.get_user_session_by_token(session_token)
    ...
    async def login(self, matric: str, password: str, email: str = None):
        ...
        session_token = await self.create_new_session(user_matric=existing_user.user_matric)
